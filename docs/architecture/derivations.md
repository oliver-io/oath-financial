...
| `post_failure_shape` | enum: `same_tool_clean_later` · `other_calls_after` · `turn_ends_on_failure`, nullable | structural | Within-turn shape after a signature match (measured: 514 / 179 / 77). Neutral positional fact; "recovered"/"fatal" interpretations are model-class. |
...
| `job_type` | enum: `doc_receipt_check` · `doc_location` · `doc_inventory` · `tie_out` · `portal_auth` · `extraction_supervision` · `drafting` · `capability_probe` · `other` | model | The line-of-business/work classification; primary product-side slice. Taxonomy is expected to be revised once the pipeline runs. |
