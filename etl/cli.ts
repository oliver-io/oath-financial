...
import { createRunContext, type RunContext, type RunContextOptions } from "./context.ts";
import { exec, runSqlFile, sqlString } from "./lib/duckdb.ts";
import { EnrichmentInvariantViolation, GateFailure, Unimplemented } from "./lib/errors.ts";
...
async function installSessionVariables(ctx: RunContext): Promise<void> {
...
async function executeStage(ctx: RunContext, stage: Stage): Promise<void> {
  ...
  try {
    ...
    for (const n of stageNumbers) {
      if (n === 3) {
        ...
        throw new Unimplemented("s3_enrich sequence", "docs/plans/etl.md §4 step 6");
      }
...
async function commandEnrich(flags: EnrichFlags, _overrides?: CliOverrides): Promise<void> {
  const jobs = flags.job === null ? enrichmentJobs : [getJob(flags.job)];
  if (jobs.some((j) => j === undefined)) {
    throw new Error(`unknown enrichment job: ${flags.job}`);
  }
  throw new Unimplemented("cli.commandEnrich", "docs/plans/etl.md §4 step 6");
}
